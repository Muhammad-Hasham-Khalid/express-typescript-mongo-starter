...
class App {
  ...
  private _initializeMiddleware(app: Express) {
    app.use(morgan('dev'));
    app.use(express.json());
    app.use(express.urlencoded({ extended: true }));
  }

  private _initializeSecurity(app: Express) {
    app.use(cors());
    app.use(helmet());
  }

  private _initializeRoutes(app: Express) {
    // health check route
  ...
  private _setupErrorHandler(app: Express) {
